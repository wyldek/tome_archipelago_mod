...
Selected prodigies may add their own rankable categories. Reviewed dependency protection can add support categories and precollect their enabling ranks. Functional dependencies prefer a compatible provider already present in the build before adding a fallback support category. Same-tree anchor ranks remain paid items, with the reviewed number of ranks requested through Archipelago's multiworld early-item pool. Precollected ranks are removed from the shuffled item pool, so every shuffled ToME item still corresponds to exactly one active ToME location.
